-- Static spell icons sourced from the bundled Babble-Spell-2.2 database.
-- Keep this optional data local so Caw does not depend on SilverDragon at runtime.
...
D.spellIconDatabaseSource="Babble-Spell-2.2"
D.spellIconFallbacks={
    -- Item summon icon confirmed by the bundled aux-addon data.
    ["Explosive Sheep"]="Interface\\Icons\\Spell_Nature_Polymorph",
    -- Explicit totem paths from the bundled Raven/Aegis totem database.
...
-- IDs observed in the bundled DPSLog captures.  These are used only when an
-- older saved breakdown predates per-spell ID storage, so the live client can
-- still resolve the texture through GetSpellInfo/Nampower.
D.spellIdFallbacks={
    ["Carve"]=51575
}
end
